...
echo "Building Make API Private Electron App..."
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    CGO_ENABLED=1 go build -ldflags="-s -w" -o make-api-private
    cd electron
    npm install
    npm run build:mac
    ...
    CGO_ENABLED=1 go build -ldflags="-s -w" -o make-api-private.exe
    cd electron
    npm install
    npm run build:win
else
