...
const Setting = () => {
  ...
  const styledPanes = panes.map((pane) => {
    const isActive = tabActiveKey === pane.itemKey;
    const tabStyle = {
      display: 'flex',
      alignItems: 'center',
      gap: '6px',
      ...
      background: isActive
        ...
        : 'transparent',
      boxShadow: isActive
        ? 'inset 0 0 0 1px var(--semi-color-primary-light-hover)'
        : 'none',
      transition: 'all 0.2s ease',
    };

    if (!React.isValidElement(pane.tab)) {
      return pane;
    }

    return {
      ...pane,
      tab: React.cloneElement(pane.tab, {
        style: {
          ...(pane.tab.props.style || {}),
          ...tabStyle,
        },
      }),
    };
